                            ...
                            .then(function (res) {
                                    ...
                                    appMeta.getData.fillDataSet(ds1, "registry", "anagrafica", filter )
                                        .then(function (dsTarget) {
                                                ...
                                                metaData.getNewRow(tRegistry.rows[0].getRow(), tChild, editType)
                                                    .then(function (row) {
                                                        expect(row).toBeDefined();
                                                        expect(row.current.idreg).toBe(1); // id chiave autoincrmento calcolato
                                                        expect(row.current.idregistryreference).toBe(3); // id
                                                        expect(tChild.rows.length).toBeGreaterThan(0); // riga aggiunta
                            ...
                            .then(function (res) {
                                    ...
                                    appMeta.getData.fillDataSet(ds1, "registry", "anagrafica", filter )
                                        .then(function (dsTarget) {
                                                ...
                                                // verifico che tornino dei dati
                                                var tRegistryAddress = dsTarget.tables["registryaddress"];
                                                var metaData = new appMeta.MetaData("registryaddress");
                                                metaData.describeColumns(tRegistryAddress, "anagraficasingle")
                                                    .then(function () {
                                                        ...
                                                        expect(tRegistryAddress.columns.address.listColPos).toBe(5);
                                                        done();
                                                    })
